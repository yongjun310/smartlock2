 ...
 *******************************************************************************/
package com.smart.lock.imageloader;
 ...
 * comparator}). And when you try to put some value into cache by key so
 * entries with "equals" keys will be removed from cache before.<br />
...
public class FuzzyKeyMemoryCache<K, V> implements MemoryCacheAware<K, V>
{
    ...
    public V get(K key)
    {
        // add by chuckcheng 20130607 图片闪烁问题
        K keyToGet = null;
        synchronized (cache)
        {
            for (K cacheKey : cache.keys())
